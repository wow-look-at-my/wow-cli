...
func TestVersion_Bare_WithBuildVersion(t *testing.T) {
	old := buildVersion
	...
	out, err := execute(t, "version", "--bare")
	require.Nil(t, err)
	assert.Equal(t, "v0.0.1234567890\n", out)
}

func TestVersion_Bare_Dev(t *testing.T) {
	old := buildVersion
	buildVersion = ""
	t.Cleanup(func() {
		buildVersion = old
...
func TestVersion_DetectsNewerLatest(t *testing.T) {
	...
	require.Nil(t, err)
	assert.Contains(t, out, "version: v0.0.1")
	assert.Contains(t, out, "latest:")
	assert.Contains(t, out, "v9.9.9")
}

func TestVersion_AlreadyLatest(t *testing.T) {
	withMockUpdater(t, "wow-cli", "v0.0.1")
	old := buildVersion
	buildVersion = "v0.0.1"
	t.Cleanup(func() { buildVersion = old })
	...
	out, err := execute(t, "version")
	require.Nil(t, err)
	assert.Contains(t, out, "v0.0.1 (latest")
}

func TestVersion_FallbackOnNoReleases(t *testing.T) {
